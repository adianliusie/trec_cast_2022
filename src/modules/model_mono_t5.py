...
# Mengjie Qian, 2022-08-22
# Modified from beir/reranking/models/mono_t5.py
...
class MonoT5:
    def __init__(self, 
                 ...
                 device = None):
        if device:
            self.device = device or next(self.model.parameters(), None).device
            self.model = self.get_model(model_path, device=self.device)
        else:
    ...
    def predict(self, sentences: List[Tuple[str,str]], batch_size: int = 32, **kwargs) -> List[float]:
        ...
        for start_idx in range(0, len(queries), 1): # Take one query at a time
