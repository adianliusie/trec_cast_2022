...
class HFDataLoader:
    ...
    def _load_corpus(self):
        if self.hf_repo:
            ...
            corpus_ds = load_dataset('json', data_files=self.corpus_file, streaming=self.streaming, keep_in_memory=self.keep_in_memory, cache_dir=self.cache_dir)
    ...
    def _load_queries(self):
        if self.hf_repo:
            ...
            queries_ds = load_dataset('json', data_files=self.query_file, streaming=self.streaming, keep_in_memory=self.keep_in_memory, cache_dir=self.cache_dir)
    ...
    def _load_qrels(self, split):
        if self.hf_repo:
            ...
            qrels_ds = load_dataset('csv', data_files=self.qrels_file, delimiter='\t', keep_in_memory=self.keep_in_memory, cache_dir=self.cache_dir)
